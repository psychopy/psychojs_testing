...
collectTestsRecursive = (label, path, results, adjustPath = true) => {
  // If this directory contains a config.json, parse it. If not, parse subdirectories
  if (fs.existsSync('config.json')) {
    ...
    try {
      ...
      throw new Error('Error in test-configuration: JSON in ' + path + '/config.json could not be parsed');
    }
    // Check if there is a labels property
    if (!config.hasOwnProperty('labels')) {
      throw new Error('Error in test-configuration: ' + path + '/config.json did not have a "labels" property');
    }
