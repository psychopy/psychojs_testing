...
flowScheduler.add(video_trialRoutineBegin());
flowScheduler.add(video_trialRoutineEachFrame());
flowScheduler.add(video_trialRoutineEnd());
...
psychoJS.start({
  ...
  expInfo: expInfo,
  resources: [
    {'name': 'resources/green_square.mp4', 'path': 'resources/green_square.mp4'}
  ]
});
...
async function experimentInit() {
  ...
  intro_text = new visual.TextStim({
    win: psychoJS.window,
    name: 'intro_text',
    text: 'wdio_video\n\nIn this test, you should see a video that is showing a green square. The video is surrounded by a red square. The video should have 1/2 of the height and 1/2 of the width of the red square. \n\nDuring the video, click anywhere to finish the experiment.\n\nClick anywhere to continue...',
    font: 'Arial',
    units: undefined, 
  ...
  // Initialize components for Routine "video_trial"
  video_trialClock = new util.Clock();
  ...
  movie = new visual.MovieStim({
    ...
    name: 'movie',
    units: undefined,
    movie: 'resources/green_square.mp4',
    pos: [0, 0],
    size: [0.5, 0.5],
    ori: 0,
    opacity: 1,
...
function video_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    gotValidClick = false; // until a click is received
    document.body.setAttribute('data-report', 'video_trial');
    // keep track of which components have finished
    video_trialComponents = [];
    video_trialComponents.push(polygon);
    video_trialComponents.push(movie);
    video_trialComponents.push(mouse_2);
    
    for (const thisComponent of video_trialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function video_trialRoutineEachFrame() {
  return async function () {
...
function video_trialRoutineEnd() {
  return async function () {
    ...
    // the Routine "video_trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
