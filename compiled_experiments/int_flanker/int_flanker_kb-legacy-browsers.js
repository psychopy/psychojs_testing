...
let expName = 'int_flanker_kb';  // from the Builder filename that created this script
let expInfo = {'': ''};
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'sheet/blocks.xlsx', 'path': 'sheet/blocks.xlsx'},
    ...
    {'name': 'sheet/trials_practice.xlsx', 'path': 'sheet/trials_practice.xlsx'},
    ...
    {'name': 'sheet/trials_main.xlsx', 'path': 'sheet/trials_main.xlsx'},
...
function load_modules_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    load_modules_trialComponents.forEach( function(thisComponent) {
...
function prepare_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    prepare_trialComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function prepare_trialRoutineEnd() {
  return async function () {
    ...
    psychoJS.experiment.addData('window_orientation', window.orientation === undefined? null: window.orientation);
    psychoJS.experiment.addData('screen_width', screen.width);
    psychoJS.experiment.addData('screen_height', screen.height);
    psychoJS.experiment.addData('window_innerWidth', window.innerWidth);
    psychoJS.experiment.addData('window_innerHeight', window.innerHeight);
...
function slideshow_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    slideshow_trialComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function pre_block_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    if (snapshot.thisN === 0) {
      rehearsing = true;
      nReps = 1;
      conditionsFile = "sheet/trials_practice.xlsx";
    } else {
      rehearsing = false;
      nReps = 4;
      conditionsFile = "sheet/trials_main.xlsx";
    }
    ...
    pre_block_trialComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function flanker_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    flanker_trialComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function feedback_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    if (deviceType === 'sm') {
        ...
        rt = flanker_keyboard.rt;
    }
    if (rt < soa) {
      feedback1 = 'img/fb_too_fast1.png';
    ...
    feedback_trialComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function post_block_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    post_block_trialComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
